...
function ChartBlockRenderer({ block, ctx }: PluginRendererProps) {
    ...
    const [dbSchemaKey, setDbSchemaKey] = useState(0)
    const [pipelineError, setPipelineError] = useState<string | null>(null)
    const [executedRows, setExecutedRows] = useState<Row[]>([])
    // Incremented AFTER column loading completes — guarantees pipeline runs with fresh column map
    const [dataVersion, setDataVersion] = useState(0)

    // Column name resolver (for chart legend labels)
    ...
    // Subscribe to ALL localdb changes to keep columns fresh
    useEffect(() => {
    ...
    // Load databases & columns — when complete, bump dataVersion to trigger pipeline
    useEffect(() => {
        rpc.call<LocalDatabase[]>('ListLocalDatabases').then(dbs => {
            ...
            setDataVersion(v => v + 1) // signal: columns are fresh, pipeline can run
        }).catch(() => { })
    }, [showEditor, refreshKey, dbSchemaKey, rpc, ctx])

    // Stable key for pipeline execution dependency tracking
    const pipelineJSON = JSON.stringify(config.pipeline)

    // Execute pipeline ONLY after column loading completes (via dataVersion).
    // This prevents running with stale column map after ETL replace.
    useEffect(() => {
        if (dataVersion === 0) return // initial render, columns not loaded yet
        ...
        const fetcher: DataFetcher = {
            ...
            getColumnMap: (dbId) => {
                const cols = dbColumnsRef.current[dbId] || []
                const map: Record<string, string> = {}
                for (const c of cols) map[c.id] = c.name
                return map
            },
        }
        executePipeline(pipeline, fetcher).then(rows => {
            setExecutedRows(rows)
            const { data, series } = rowsToChart(rows, pipeline.viz, resolveCol)
            persist({ ...config, pipeline, data, series })
        }).catch(err => {
            setPipelineError(String(err))
        })
    }, [pipelineJSON, dataVersion])
